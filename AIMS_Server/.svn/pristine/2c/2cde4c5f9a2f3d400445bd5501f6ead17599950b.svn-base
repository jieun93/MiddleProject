package kr.or.ddit.vo;

import java.io.Serializable;

public class CalendarVO implements Serializable{

	/**
	 * 
	 */
	private static final long serialVersionUID = 2185376650248375070L;
	private String 	 CAL_NUM;
	private String 	 CAL_CONN;
	private String 	 CAL_TITLE; // 달력 내용 넣는거 
	private String 	 CAL_SDATE;
	private String 	 CAL_EDATE;
	
	public CalendarVO() {}

	public CalendarVO(String cAL_NUM, String cAL_CONN, String cAL_TITLE, String cAL_SDATE, String cAL_EDATE) {
		super();
		this.CAL_NUM = cAL_NUM;
		this.CAL_CONN = cAL_CONN;
		this.CAL_TITLE = cAL_TITLE;
		this.CAL_SDATE = cAL_SDATE;
		this.CAL_EDATE = cAL_EDATE;
	}

	public String getCAL_NUM() {
		return CAL_NUM;
	}

	public void setCAL_NUM(String cAL_NUM) {
		CAL_NUM = cAL_NUM;
	}

	public String getCAL_CONN() {
		return CAL_CONN;
	}

	public void setCAL_CONN(String cAL_CONN) {
		CAL_CONN = cAL_CONN;
	}

	public String getCAL_TITLE() {
		return CAL_TITLE;
	}

	public void setCAL_TITLE(String cAL_TITLE) {
		CAL_TITLE = cAL_TITLE;
	}

	public String getCAL_SDATE() {
		return CAL_SDATE;
	}

	public void setCAL_SDATE(String cAL_SDATE) {
		CAL_SDATE = cAL_SDATE;
	}

	public String getCAL_EDATE() {
		return CAL_EDATE;
	}

	public void setCAL_EDATE(String cAL_EDATE) {
		CAL_EDATE = cAL_EDATE;
	}
	
	
	
	
	
	
}
